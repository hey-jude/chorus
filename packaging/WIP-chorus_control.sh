...
function start () {
  ...
  if  should_handle "--clean" || should_handle "-c" ; then
    clean_cache
  fi 
...
function clean_cache () {
    ...
    CHORUS_CACHE=$CHORUS_HOME/tmp/cache
    
    printf "Cleaning application cache.."
    # $CHORUS_HOME/tmp/cache/assets/
    rm -rdf $CHORUS_CACHE/assets/
    printf "."
    # $CHORUS_HOME/tmp/cache/chorus/
    rm -rdf $CHORUS_CACHE/chorus/
    printf "."
    # $CHORUS_HOME/tmp/cache/sass/
    rm -rdf $CHORUS_CACHE/sass/
    printf ".\n"
}
...
function usage () {
  ...
  echo
  echo Usage:
  echo "  $script start   [services] [-clean]                       start services"
  echo "  $script stop    [-t max_wait_time] [services]             stop services"
  echo "  $script restart [-t max_wait_time] [services] [-clean]    stop and start services"
  ...
  echo "  $script start -clean               start all services, clearing the entire app cache before starting"
