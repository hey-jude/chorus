  ...
  task :workspaces => :environment do
    ...
    users.each do | user|
      ...
      workspaces.each do |workspace|
        options = {:user => user,:succinct => true, :show_latest_comments => true, :cached => true, :namespace => "home:workspaces"}
        ApiPresenter.present(workspace, nil, options)
  ...
  task :activities => :environment do
    ...
    users.each do | user|
      ...
      events.each do |event|
        ApiPresenter.present(event, nil, options)
        print "."
        $stdout.flush
      end
      printf " done\n"
    ...
    users.each do | user|
      ...
      workspaces.each do |workspace|
        ...
        if datasets != nil && datasets.size != 0
          ApiPresenter.present(datasets, nil, options)
          print "."
        else
          print "x"
        end
  ...
  task :workfiles => :environment do
    ...
    users.each do | user|
      ...
      workspaces.each do |workspace|
        ...
        ApiPresenter.present(workfiles, nil, options)
        print "."
        $stdout.flush
      end
      printf " done\n"
