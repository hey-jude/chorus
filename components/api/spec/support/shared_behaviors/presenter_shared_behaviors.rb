...
shared_examples "dataset presenter with workspace" do |dataset_factory_name|
  ...
  context "when the 'workspace' option is passed" do
    ...
    it "includes the given workspace" do
      hash[:workspace].should == ApiPresenter.present(workspace, view)
    end
  end
end
...
shared_examples :succinct_data_source_presenter do
  ...
  it "should only present a subset" do
    extra_keys = presenter.presentation_hash.keys - [:id, :name, :entity_type, :is_deleted, :owner_id, :shared, :version, :supports_work_flows, :hdfs_version, :is_hawq, :hive_metastore_location, :is_hdfs_hive, :state]
