...
describe Api::SessionPresenter, :type => :view do
  let(:user) { users(:owner) }
  let(:session) { Session.create!(:username => user.username, :password => SPEC_PASSWORD) }
  let(:options) { {} }
  let(:presenter) { Api::SessionPresenter.new(session, view, options) }
  before(:each) do
  ...
  describe "#to_hash" do
    ...
    it "uses the user presenter to present the user" do
      hash[:user].should == (Api::UserPresenter.new(session.user, view).presentation_hash)
    end
  end
