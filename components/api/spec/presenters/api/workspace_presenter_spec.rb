...
describe Api::WorkspacePresenter, :type => :view do
  ...
  describe "#to_hash" do
    ...
    it "uses the image presenter to serialize the image urls" do
      hash[:image].to_hash.should == (Api::ImagePresenter.new(workspace.image, view).presentation_hash)
    end
    ...
    it 'should use UserPresenter presentation_hash method for owner' do
      ...
      new_owner.to_hash.should == (Api::UserPresenter.new(user, view).presentation_hash)
    end

    it 'should use UserPresenter presentation_hash method for archiver' do
      new_archiver = hash[:archiver]
      new_archiver.to_hash.should == (Api::UserPresenter.new(archiver, view).presentation_hash)
    end

    it "should use gpdbSchemaPresenter Hash method for sandbox_info" do
      new_sandbox = hash[:sandbox_info]
      new_sandbox.to_hash.should == (Api::GpdbSchemaPresenter.new(schema, view, {:succinct => true}).presentation_hash)
    end
    ...
    context "when the workspace has tags" do
      ...
      it 'includes the tags' do
        ...
        hash[:tags].should == ApiPresenter.present(workspace.tags, @view)
      end
    end
