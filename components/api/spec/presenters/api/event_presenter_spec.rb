...
describe Api::EventPresenter, :type => :view do
  ...
  describe "#simple_hash" do
    subject { Api::EventPresenter.new(event, view, options).simple_hash }
    let(:options) { {} }
  ...
  describe "#to_hash" do
    subject { Api::EventPresenter.new(event, view, options) }
    let(:options) { {} }
    ...
    context "when rendering the activity stream" do
      ...
      context "SourceTableCreated" do
        ...
        it "does not render datasets with their schemas or associated workspaces" do
          ...
          hash[:dataset].should == Api::DatasetPresenter.present(dataset, view, sub_presenter_options)
      ...
      context "NoteOnWorkspace" do
        ...
        it "presents the succinct hash of the workspace" do
          hash = subject.to_hash
          hash[:workspace].should == Api::WorkspacePresenter.new(workspace_with_sandbox, view, sub_presenter_options).to_hash
        end
      end
    end
    ...
    context "Non-note event" do
      ...
      it "includes the 'id', 'timestamp', 'actor', 'action'" do
        ...
        hash[:actor].should == ApiPresenter.present(event.actor, view, sub_presenter_options)
      end
      ...
      it "presents all of the event's 'targets', using the same names" do
        ...
        hash = subject.to_hash
        hash[:special_data_source].should == ApiPresenter.present(special_data_source, view, sub_presenter_options)
        hash[:special_user].should == ApiPresenter.present(special_user, view, sub_presenter_options)
      end
    ...
    context "Note event" do
      ...
      context "with an attachment" do
        ...
        it "contains the attachment" do
          ...
          hash[:attachments][1][:workspace].should == event.workspace.attributes
    ...
    context "events that are insight" do
      ...
      it "has hash for insights" do
        ...
        hash[:promoted_by].should == ApiPresenter.present(user, view, sub_presenter_options)
    ...
    context "events with errors" do
      ...
      it "presents the errors" do
        hash = subject.to_hash
        hash[:error_objects].should == Api::ErrorPresenter.new(event.error_objects).as_json
      end
    end
    ...
    context "presenting as a comment" do
      ...
      context "for a note" do
        ...
        it "should present a restricted set" do
          ...
          hash[:author].should == Api::UserPresenter.new(event.actor, view, sub_presenter_options).presentation_hash
          hash[:timestamp].should == event.created_at
