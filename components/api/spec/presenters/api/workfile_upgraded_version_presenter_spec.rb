...
describe Api::WorkfileUpgradedVersionPresenter, :type => :view do
  let(:current_user) { users(:owner) }
  before { set_current_user(current_user) }

  let(:event) { Events::WorkfileUpgradedVersion.first }
  subject { Api::WorkfileUpgradedVersionPresenter.present event, view, options }
  let(:options) { {} }
  let(:hash) { subject.to_hash }
  let(:workfile_hash) { ApiPresenter.present(event.workfile, view, sub_presenter_options.merge(:workfile_as_latest_version => true)).to_hash }
  let(:workspace_hash) { ApiPresenter.present(event.workspace, view, sub_presenter_options).to_hash }
  ...
  it 'includes all the relevant fields' do
    ...
    hash[:actor].should == Api::UserPresenter.new(event.actor, view, sub_presenter_options).presentation_hash
