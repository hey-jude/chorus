...
describe Api::ChorusWorkfilePresenter, :type => :view do
  let(:user) { users(:owner) }
  let(:workfile) { workfiles(:private) }
  let(:workspace) { workfile.workspace }
  let(:options) { {} }
  let(:presenter) { Api::ChorusWorkfilePresenter.new(workfile, view, options) }
  ...
  describe "#to_hash" do
    ...
    context ":include_execution_schema is passed as an option" do
      ...
      it "includes the execution_schema" do
        hash[:execution_schema].should == Api::GpdbSchemaPresenter.new(workfile.execution_schema, view).presentation_hash
      end
    end
    ...
    context "for list view" do
      ...
      context "when also presenting the execution schema" do
        ...
        it "should present the execution schema succinctly" do
          hash[:execution_schema].should == Api::GpdbSchemaPresenter.new(workfile.execution_schema, view, :succinct => true).presentation_hash
        end
      end
    end
    ...
    describe "when the 'workfile_as_latest_version' option is set" do
      ...
      it "calls the presenter for the latest version of the workfile" do
        mock(Api::ChorusWorkfilePresenter).present(workfile.latest_workfile_version, anything, {})
        hash
      end
      ...
      context "when there is no latest workfile version" do
        ...
        it "does not try to present the latest workfile version" do
          dont_allow(ApiPresenter).present
          hash
        end
      end
    end
