...
describe Api::DatasetPresenter, :type => :view do
  module Api
  ...
  let(:succinct) { false }
  let(:presenter) { Api::WithTableauPresenter.new(dataset, view, {:workspace => workspace, :activity_stream => activity_stream, :succinct => succinct}) }
  ...
  describe '.tableau_hash' do
    ...
    context "when the presenter doesn't support tableau workbooks" do
      let(:presenter) { Api::DatasetPresenter.new(dataset, view, {:workspace => workspace, :activity_stream => activity_stream}) }
    ...
    context 'for a sandbox table' do
      ...
      context 'when the table has tags' do
        ...
        it 'includes tags' do
          ...
          hash[:tags].should == ApiPresenter.present(dataset.tags, @view)
        end
      end
