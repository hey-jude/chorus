...
describe Api::WorkfileVersionPresenter, :type => :view do
  let(:workfile) { workfiles(:public) }
  let(:owner) { workfile.owner }
  let(:version) { workfile.latest_workfile_version }
  let(:presenter) { Api::WorkfileVersionPresenter.new(version, view, options) }
  let(:options) { {} }
  ...
  describe "#to_hash" do
    ...
    it "presents the owner and modifier succinctly" do
      hash[:owner].to_hash.should == Api::UserPresenter.new(owner, view, :succinct => true).presentation_hash
      hash[:modifier].to_hash.should == Api::UserPresenter.new(owner, view, :succinct => true).presentation_hash
    end
