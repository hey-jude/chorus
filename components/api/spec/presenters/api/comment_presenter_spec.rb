...
describe Api::CommentPresenter, :type => :view do
  let(:comment) { comments(:comment_on_note_on_greenplum) }
  let(:presenter) { Api::CommentPresenter.new(comment, view) }
  ...
  describe "#to_hash" do
    ...
    it "presents a succinct author" do
      hash[:author].to_hash.should == (Api::UserPresenter.new(comment.author, view, :succinct => true).presentation_hash)
    end
