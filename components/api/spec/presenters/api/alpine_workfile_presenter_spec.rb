...
describe Api::AlpineWorkfilePresenter, :type => :view do
  let(:user) { users(:owner) }
  let(:workfile) { workfiles(:'alpine_flow') }
  let(:options) { {} }
  let(:presenter) { Api::AlpineWorkfilePresenter.new(workfile, view, options) }
    ...
    describe "execution locations" do
      context "when presenting for a 'show' view" do
        it "presents execution location" do
          hash[:execution_locations].should == workfile.execution_locations.map { |loc| ApiPresenter.present(loc, view, :succinct => true) }
        end
      end
