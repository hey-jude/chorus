...
describe Api::WorkfilePresenter, :type => :view do
  let(:user) { users(:owner) }
  let(:workfile) { workfiles(:private) }
  let(:workspace) { workfile.workspace }
  let(:options) { {} }
  let(:presenter) { Api::WorkfilePresenter.new(workfile, view, options) }
  ...
  describe "#to_hash" do
    ...
    it "uses the workspace presenter to serialize the workspace" do
      hash[:workspace].to_hash.should == (Api::WorkspacePresenter.new(workspace, view).presentation_hash)
    end

    it "the workspace presenter does not lose the succinct option" do
      options[:succinct] = true
      hash[:workspace].to_hash.should == (Api::WorkspacePresenter.new(workspace, view, :succinct => true).presentation_hash)
    end

    it "uses the succinct user presenter to serialize the owner" do
      hash[:owner].to_hash.should == (Api::UserPresenter.new(user, view, :succinct => true).presentation_hash)
    end
    ...
    context "when the workfile has tags" do
      ...
      it 'includes the tags' do
        ...
        hash[:tags].should == ApiPresenter.present(workfile.tags, @view)
      end
    end
    ...
    context "when there are notes on a workfile" do
      ...
      it "presents the notes as comments in reverse timestamp order" do
        recent_comments[0][:author].to_hash.should == ApiPresenter.present(user, view, {:succinct => true, :activity_stream => true})
      ...
      context "when there is a comment on a note" do
        ...
        context "when the comment is newer than the notes" do
          it "presents the comment before the notes" do
            recent_comments[0][:author].to_hash.should == ApiPresenter.present(user, view, :succinct => true)
