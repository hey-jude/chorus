...
describe ApiPresenter, :type => :view do
  before do
    @model = FactoryGirl.build :user
    @presenter = ApiPresenter.new(@model, view)
  end
  ...
  describe ".present_collection" do
    it "serializes an array" do
      ApiPresenter.present_collection([@model], view, {}).should be_a(Array)
    end

    it "allows passing explicit presenter class for polymorphic collections" do
      class Api::SpecExplicitPolymorphicPresenter < ApiPresenter
        def to_hash
          {
            :name => model.name
          }
      ...
      hash = ApiPresenter.present_collection([GpdbTable.new(:name => 'foo'), GpdbView.new(:name => 'bar')], view, {:presenter_class => 'SpecExplicitPolymorphicPresenter'})
      hash.should == [{:name => 'foo'}, {:name => 'bar'}]
    end
  end

  describe ".present" do
    let(:json) { ApiPresenter.present(object_to_present, view) }

    context "with a single model" do
      let(:object_to_present) { FactoryGirl.build(:user) }

      context "when forbidden is true" do
        let(:json) { ApiPresenter.present(object_to_present, view, {:forbidden => true}) }
      ...
      it "passes its options on" do
        mock(ApiPresenter).present_model(object_to_present, { view: true }, { test: true })
        ApiPresenter.new(object_to_present, { view: true }).present(object_to_present, { test: true })
      end
      ...
      context "when complete_json? is true" do
        before do
          any_instance_of(Api::UserPresenter, :complete_json? => true)
        end
      ...
      context "when complete_json? is false" do
        before do
          any_instance_of(Api::UserPresenter, :complete_json? => false)
        end
    ...
    context "with a paperclip attachment" do
      it "creates an ImagePresenter" do
        mock.proxy(Api::ImagePresenter).new(@model.image, view, {})
        ApiPresenter.present(@model.image, view)
      end
    end
    ...
    context "with a subclass of Events::Base" do
      it "creates an EventPresenter" do
        ...
        mock.proxy(Api::EventPresenter).new(event, view, {})
        ApiPresenter.present(event, view)
      end
    end
    ...
    context "with an acts as taggable on tag" do
      it "should create a TagPresenter" do
        ...
        mock.proxy(Api::TagPresenter).new(event, view, {})
        ApiPresenter.present(event, view)
      end
    end
