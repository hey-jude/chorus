...
class ApiController < ApplicationController
  ...
  def set_collection_defaults
    ...
    params.reverse_merge!(Rails.configuration.collection_defaults)
    request.params.reverse_merge!(Rails.configuration.collection_defaults)
  end

  def present(model_or_collection, options={})
    presenter_options = options.delete(:presenter_options) || {}
    json = {:response => ApiPresenter.present(model_or_collection, view_context, presenter_options)}
  ...
  def present_forbidden(model, error_type = nil)
    ...
    if model
      response_json[:errors] = {
        :model_data => ApiPresenter.present(model, view_context, :forbidden => true)
      }
    end
  ...
  def present_validation_errors(errors, options={})
    present_errors({:fields => Api::ErrorPresenter.new(errors)}, options)
  end
