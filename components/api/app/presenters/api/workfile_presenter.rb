module Api
  class WorkfilePresenter < ApiPresenter
    def to_hash
      ...
      workfile.merge!({
          :associated_worklets => present(Worklet.where("content_type='worklet' AND additional_data SIMILAR TO '%(,|{)\"workflow_id\":#{ model.id.to_s }(,|})%'").order(:file_name))
      })
