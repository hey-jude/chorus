...
module MariadbIntegration
  ...
  def self.setup_test_schemas
    refresh_if_changed do
      ...
      sql = ERB.new(File.read(Core::Engine.root.join 'spec/support/database_integration/setup_mariadb_databases.sql.erb')).result(binding)
  ...
  def self.drop_test_database
    ...
    sql = ERB.new(File.read(Core::Engine.root.join 'spec/support/database_integration/drop_mariadb_databases.sql.erb')).result(binding)
    execute_sql(sql)
  end
