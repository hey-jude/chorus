...
describe("chorus.pages.AboutThisApp", function() {
    beforeEach(function () {
    ...
    describe("when the fetch for the license completes", function() {
        beforeEach(function () {
            this.license = backboneFixtures.license();
            this.server.completeFetchFor(this.dialog.model, this.license);
        });



        it("includes the application name", function () {
            expect(this.dialog.$("#application_title")).toContainTranslation("about." + this.license.applicationKey());
        });
        ...
        describe("when the version string is returned", function () {
            ...
            it("displays the version info", function () {
                expect(this.dialog.$("#application_version")).toContainText(this.version);
            });
        });
    });
    ...
    describe("when the license vendor is: alpine", function () {
        ...
        it("renders the license info", function () {
            _.each(["collaborators", "admins", "developers", "expires"], function(item) {
                expectItemExists(this.dialog, this.license, item);
            }, this);
        });
    });
    ...
    describe("when the license vendor is: openchorus", function () {
        ...
        it("displays only the version info", function () {
            _.each(["collaborators", "admins", "developers", "expires"], function(item) {
                expectItemDoesNotExist(this.dialog, this.license, item);
            }, this);
        });
    });
    ...
    describe("when the license vendor is pivotal", function () {
        ...
        it("displays only the version info", function () {
            ...
            expectItemExists(this.dialog, this.license, "expires");
        });
    });

    function expectItemExists(dialog, license, item) {
        expect(dialog.$(".items ." + item)).toContainText(t("about." + item) + ": " + license.get(item));
    }

    function expectItemDoesNotExist(dialog, license, item) {
        expect(dialog.$(".items ." + item)).not.toContainText(t("about." + item) + ": " + license.get(item));
    }
});
